-- Story 6.6 — Migration 0051: extend audit_append_external allowlist for
-- the sms.resend_initiated event.
--
-- The Story 6.5 baseline (migration 0046,
-- 20260501000003_audit_append_external_extend_optout.sql) shipped the
-- allowlist as {sms.queued, sms.sent, sms.failed, sms.abandoned, sms.opt_out}.
-- Story 6.6 adds 'sms.resend_initiated'. Function body is byte-for-byte
-- identical EXCEPT the allowlist line + comment line — same diff discipline
-- as 6.2 / 6.5.
--
-- Note: epics.md line 1069 (Story 6.7 BDD) mentions the literal event
-- name 'sms.resend'; the canonical event name introduced here is
-- 'sms.resend_initiated' to match the existing namespace pattern (verb
-- in past participle). Story 6.7 will reuse this same event with a
-- different payload shape ({transaction_id, member_id} vs the Story 6.6
-- payload {member_id, cycle_id, count}).
--
-- See: _bmad-output/implementation-artifacts/6-6-resend-cycle-history.md AC #2.
...
begin
  ...
  if p_event_type not in ('sms.queued', 'sms.sent', 'sms.failed', 'sms.abandoned', 'sms.opt_out', 'sms.resend_initiated') then
...
comment on function public.audit_append_external(text, uuid, text, jsonb) is
  'Story 6.1/6.2/6.5/6.6 — SECURITY DEFINER helper for non-trigger audit events. Mirrors audit_emit canonical serialisation byte-for-byte. Allowed event_types: sms.queued (Story 6.1 — manual sms-dispatch), sms.sent / sms.failed / sms.abandoned (Story 6.2 — sms-worker terminal states), sms.opt_out (Story 6.5 — saver consent withdrawal), sms.resend_initiated (Story 6.6 — collector-initiated cycle/transaction resend).';
