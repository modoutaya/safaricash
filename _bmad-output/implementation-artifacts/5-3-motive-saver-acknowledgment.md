...
- [ ] **Task 0 — Component extension (AC #1 #2 #3 #4 #5 #7).** Edit `src/features/transaction/ui/AdvanceFlow.tsx`:
...
- [ ] **Task 1 — Replace Story 5.2 placeholder tooltip (AC #4 #8).** Remove the `advance.flow.cta_disabled_tooltip` key from `fr.json` and replace the JSX with the contextual tooltip helper.

- [ ] **Task 2 — i18n keys (AC #8).** Add 7 keys; remove 1.

- [ ] **Task 3 — Tests (AC #9).** Extend `src/features/transaction/ui/AdvanceFlow.test.tsx` with ≥ 10 cases.

- [ ] **Task 4 — All gates (AC #11).** `typecheck` / `lint` / `test --coverage` / `build`.

- [ ] **Task 5 — Hygiene + status flip.**
