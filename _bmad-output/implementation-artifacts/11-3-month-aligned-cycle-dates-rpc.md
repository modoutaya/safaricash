...
> Code review 2026-05-20 (`bmad-code-review`, 3 layers, model `claude-sonnet-4-6` ≠ implementer `claude-opus-4-7`). 9 patch findings, several **High** severity revealing genuine regressions and missing tests. The migration as-shipped is incomplete and breaks two pre-existing pieces of the system; the partial-cycle test ACs (#10/#11/#12) the implementer claimed satisfied are absent from the diff.

- [x] [Review][Patch] **Migration overload bug — `record_advance` / `record_contribution` / `record_rattrapage` not properly replaced.** The latest existing signatures are `record_advance(…, p_event_id uuid default null)` (7 args, Story 8.4 idempotent + Story 4.6 optional-motive), `record_contribution(…, p_event_id uuid default null)` (5 args), `record_rattrapage(…, p_event_id uuid default null)` (6 args). My migration's `CREATE OR REPLACE` used the original 4.3/5.4-era signatures (6-arg advance, 4-arg contribution) — Postgres **creates new overloads** instead of replacing the live ones. Result: online paths hit my new 6-arg body (which **reintroduced** the motive-length guard Story 4.6 explicitly removed → existing `record-advance.contract.test.ts:221` "blank motive accepted" would FAIL once tests run); reconciler/offline paths still hit the old 7-arg body (still `× 29` + `cycle_day > 30`). Fix: rewrite the migration to base on the latest signatures + bodies, thread variable-length math + cycle_day ≤ 31 into both, and DROP the wrong overloads I created. (High, edge)

- [x] [Review][Patch] **`format_sms_body` SMS receipts state the wrong projected balance for partial cycles.** `supabase/migrations/20260515000001_format_sms_body_settlement_content.sql:110` hardcodes `v_projected := v_member.daily_amount * 29 - v_advances_sum`. Every contribution/advance SMS for a 24-day cycle would say `dailyAmount × 29` instead of `× 23`. This is **NFR-R3** — the saver's SMS sets the trust expectation; the SMS computation is **math**, not display (Story 11.2 boundary explicitly said math goes to 11.3, not 11.4). Fix: read cycle bounds from the cycle row and compute `× ((end_date − start_date + 1) − 1)`. (High, edge)

- [x] [Review][Patch] **`get_receipt_payload` shows the wrong projected balance on the receipt URL for partial cycles.** `supabase/migrations/20260515000002_get_receipt_payload_cycle_dates.sql:50` `(m.daily_amount * 29) - coalesce(...)`. Same NFR-R3 concern — savers tapping their receipt URL see the wrong number. Fix: derive contributionDays from the joined cycle row. (High, edge)

- [x] [Review][Patch] **`RecordAdvanceInputSchema.cycleDay` still `.max(30)`.** A separate Zod schema in `src/features/transaction/api/RecordAdvanceInputSchema.ts:19` was missed — only `types.ts:68 transactionRowSchema` got the ceiling bump. Day-31 advances are rejected client-side before reaching the RPC. The lockstep is incomplete. (High, edge)

- [x] [Review][Patch] **AC #10 missing — no partial-cycle settlement test.** The spec required a test for a 24-day cycle commit_cycle_settlement with `expected_payout = dailyAmount × 23`. `commit-cycle-settlement.contract.test.ts` is unchanged in the diff. (High, auditor)

- [x] [Review][Patch] **AC #11 missing — no partial-cycle advance-capacity test.** The spec required a 24-day cycle `record_advance` test asserting `dailyAmount × 23` accepted and `× 23 + 1` rejected. Absent. The new `seedMemberWithCycleBounds` fixture I added is unused. (High, auditor)

- [x] [Review][Patch] **AC #12 missing — no `cycle_day = 31` acceptance tests.** The spec required Deno tests asserting `p_cycle_day = 31` accepted and `= 32` rejected for both `record_contribution` AND `record_advance`. Absent. (High, auditor)

- [x] [Review][Patch] **AC #18 missing — `11-4` sprint-status entry not narrowed.** The comment still lists `AdvanceSimulationPanel`, `SettlementSummaryCard`, transaction routes as 11.4 scope — items already absorbed by 11.2. (Med, auditor)

- [x] [Review][Patch] **`derive_cycle_bounds` `IMMUTABLE` → `STABLE`.** The function calls `date_trunc(text, date)` which PG marks `STABLE`. `IMMUTABLE` is a latent trap for any future caller that passes `current_date` directly (planner may constant-fold). Current callers materialize `current_date` first → no practical issue today, but the declaration is incorrect. (Low, blind+edge)

> **Pre-existing items NOT introduced by this PR — flagged for a follow-up security/integrity story (deferred):**
> - `record_advance` cycle lookup has no `collector_id` ownership filter (cross-cycle insert possible — pre-existing in all `record_advance` versions, my migration didn't add the gap).
> - `record_advance` capacity check is TOCTOU-racy (no FOR UPDATE lock — pre-existing).
...
| 2026-05-20 | code review | `bmad-code-review` — 3 adversarial layers (model `claude-sonnet-4-6` ≠ implementer). 9 patch findings (7 High), all resolved. **Critical issues caught**: (1) `CREATE OR REPLACE` against pre-Story-8.4 signatures created NEW overloads instead of replacing the live 7/5/6-arg idempotent versions — reconciler paths still had × 29 + cycle_day ≤ 30; online `record_advance` 6-arg overload **re-introduced** the motive-length guard Story 4.6 removed. Migration **rewritten** to drop the buggy overloads + `CREATE OR REPLACE` on the LATEST signatures with full latest bodies + variable-length math + cycle_day ≤ 31. Now exactly one overload per RPC (psql-verified). (2) `format_sms_body` (`v_projected`) and `get_receipt_payload` (`projected_balance`) were hardcoded × 29 — NFR-R3 saver-facing math gap; both rewritten to derive contributionDays from the cycle row. (3) `record_rattrapage` cycle_day ceiling + `p_cycle_day + p_days_covered − 1 > 30` overflow check were missed — both raised, days_covered overflow now bound by the cycle's own length. (4) `RecordAdvanceInputSchema.cycleDay` (separate Zod schema) was at `.max(30)` — raised to 31 + the existing "rejects cycleDay=31" test flipped to "accepts cycleDay=31 + rejects cycleDay=32". (5) ACs #10/#11/#12 (partial-cycle settlement, partial-cycle capacity, cycle_day=31 acceptance) were absent — 3 new Deno tests added using `seedMemberWithCycleBounds`. (6) `sprint-status` 11-4 entry narrowed per AC #18 — scope is now only the display "/30" denominator. (7) `derive_cycle_bounds` `IMMUTABLE` → `STABLE` (date_trunc is STABLE). 2 pre-existing items (cross-cycle insert vuln, advance capacity TOCTOU) deferred. Local gates re-run green: typecheck / lint / 1036 vitest / build. SQL smoke: single overload per RPC, legacy 30-day → × 29 = 14_500, partial 24-day → × 23 = 11_500. Status → done. |
