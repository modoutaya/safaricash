...
- [ ] **Task 0 — Component (AC #1 #2 #3 #4 #6 #8 #10).** Create `src/components/domain/AdvanceSimulationPanel.tsx`. Pure functional component. Compute `state` once at the top of the body. Render 4 rows + the optional explanatory note for over-limit.

- [ ] **Task 1 — i18n keys (AC #9).** Add 7 keys under `advance.simulation.*` to `src/i18n/fr.json`. The `TranslationKey` derivation picks them up.

- [ ] **Task 2 — Tests (AC #12).** Create `src/components/domain/AdvanceSimulationPanel.test.tsx`. ≥ 7 cases. Use `jest-axe` for the a11y assertion.

- [ ] **Task 3 — All gates (AC #17).** `typecheck` / `lint` / `test --coverage` / `build`.

- [ ] **Task 4 — Hygiene + status flip.**
