...
- All 13 ACs satisfied. FR17 formula isolated in `computeMemberStats` (pure, 100% covered). Hook fans out 3 parallel PostgREST queries against `members_decrypted` / `cycles` / `transactions_decrypted` — same pattern as Story 2.1. The presentation `MemberProfile` is pure (zero data-fetching), so the route hosts loading / error / not-found / success branching.
- Action overflow menu shipped as 3 disabled placeholder buttons (Modifier / Redémarrer / Supprimer) — Stories 2.5/2.6/2.7 will flip the `disabled` flag.
- `useMembers.pickCurrentCycle` heuristic re-implemented locally for hook isolation; consolidating to a shared helper is a 1-file grep when needed.
- Coverage gates green (75.7% branches > 75% threshold). 19 new Story 2.4 tests added (9 stats + 4 time-format + 1 icon + 5 hook + 6 component + 5 route + 1 navigation regression on MemberList).
- E2E `flow-2-member-profile.spec.ts` written, env-gated on `SUPABASE_TEST_SEED_READY` (matches Story 2.1's pattern).
- Components extracted to module-level (`ProfileNotFound` / `ProfileError` / `ProfileSkeleton`) to satisfy `react-hooks/static-components`.

### Debug Log

- **Zod v4** — `z.number({ invalid_type_error: ... })` is no longer supported; switched to `.int(message)`.
- **Mock-shape iteration on `useMemberProfile.test.tsx`** — initial `then(resolve)` chain on the supabase mock didn't survive the 3-parallel-query restructure; fixed by switching to a `from(table)` switch returning `eq` + `maybeSingle` per table, then exposing `fetchProfile` for direct testing.
- **UUID parsing** — Zod `.uuid()` rejected `"c-1"`-style fixtures; replaced all test ids with proper v4 UUIDs.
- **`@typescript-eslint/consistent-type-imports`** — `vi.importActual<typeof import("...")>(...)` was flagged for `import()` type annotation; refactored to a top-level `import type * as MemberModule from ...`.

## File List

**New:**
- `src/features/member/api/computeMemberStats.ts` (+ test)
- `src/features/member/api/formatTransactionTime.ts` (+ test)
- `src/features/member/api/transactionIcon.ts` (+ test)
- `src/features/member/api/useMemberProfile.ts` (+ test)
- `src/features/member/ui/MemberProfile.tsx` (+ test)
- `src/app/routes/members/[id].tsx` (+ test)
- `tests/e2e/flow-2-member-profile.spec.ts`

**Modified:**
- `src/features/member/types.ts` (added `transactionRowSchema`, `cycleRowSchema`, `MEMBER_PROFILE_QUERY_KEY`, `MemberStats`)
- `src/features/member/index.ts` (barrel exports for the new hook + component + types)
- `src/features/member/ui/MemberList.tsx` (wired `onSelect → navigate(/members/:id)`)
- `src/features/member/ui/MemberList.test.tsx` (+ navigation regression test)
- `src/app/router.tsx` (registered `/members/:id` after `/new` and `/import` for static-path precedence)
- `src/i18n/fr.json` (added `members.profile.*` namespace, ~18 keys)
...
| 2026-04-22 | dev agent | Implementation complete. All 13 ACs satisfied, 19 new tests, full gates green (typecheck / lint / 336 vitest / build / coverage 75.7% > 75%). Status → review. |
