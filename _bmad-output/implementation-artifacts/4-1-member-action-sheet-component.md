...
As a **developer**,
I want **a reusable `MemberActionSheet` component that opens as a centered modal with the member's pre-suggested amount**,
...
7. **Modal behaviour — list visible behind.** **And** the list behind the sheet remains visible (`epics.md:804`). The sheet uses the native `<dialog>` element (consistent with Stories 2.6 + 2.7) with `backdrop:bg-neutral-900/50` (semi-transparent overlay). The sheet is centered horizontally and vertically (`m-auto`) with fully rounded corners and the list shows through the backdrop. *(Amendment 2026-05-14: was bottom-anchored in the original spec; converted to centered modal — see Change Log.)*
...
- [ ] **Task 0 — Component skeleton (AC #2 #3 #4 #5 #7 #9 #10).** Create `src/components/domain/MemberActionSheet.tsx`:
  ...
  - Centered layout via Tailwind utilities (`m-auto`, `rounded-2xl`).
...
- All 15 ACs satisfied. `MemberActionSheet` lives at `src/components/domain/`. Native `<dialog>` shell, centered (was bottom-anchored at story-creation time — amended 2026-05-14, see Change Log). 4 transaction CTAs disabled by default (Stories 4.3-4.5 will wire). Closed-cycle gate via Story 3.4 helper.
...
| 2026-05-14 | Mamadou (founder)   | **Amendment — bottom-sheet → centered modal.** Converted the `<dialog>` from bottom-anchored (`m-auto mb-0` + `rounded-t-2xl rounded-b-none border-x border-t`) to centered (`m-auto` + `rounded-2xl border`). Reason: bottom-sheet was perceived as cut-off / non-centered on the dev viewport during local testing post-Story 7.2. Scope is **Flow 1 only** — Flow 2 *Prêt* preamble, Flow 4 dispute confirmation, and the general bottom-sheet pattern in the UX spec (l. 918, l. 1363, l. 1416) are unchanged. Divergence note added to UX spec § Flow 1 and § Member Action Sheet (l. 1002). Files touched: `src/components/domain/MemberActionSheet.tsx` (className + header comment), `_bmad-output/planning-artifacts/ux-design-specification.md`, this story file. No code-test impact (no test asserted on the bottom-anchor classes). |
